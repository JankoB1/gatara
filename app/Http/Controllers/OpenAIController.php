...
//                "max_tokens" => 250,
    ...
    public function getChatGPTResponseBa(Request $request) {
        try {
            if(!$this->checkHeartbeat($request)) {
                return response()->json(['error' => 'Heartbeat failed'], 401);
            }
            ...
            $url = 'https://api.openai.com/v1/chat/completions';
            $content = 'Napiši izmišljeno proricanje sudbine na bosanskom jeziku u prvom ženskom licu iz oblasti ' . $theme . '. Osoba kojoj pišeš sudbinu ima sljedeće informacije: ' . $age . ' godina, ' . $gender . ', ' . $relationship . ', ' . $jobStatus . '. Potrebno je da prva rečenica počinje sa "Dragi ' . $name . ' ,\n" ili "Draga ' . $name . ' ,\n"ovisno o tome je li osoba muško ili žensko. Obavezno je da postoje tačno 3 pasusa i da svaki pasus ima po 2-3 kraće rečenice. U svakom pasusu treba da \'vidiš u šolji\' po jedan od simbola: ' . $symbols[0]->name . ', ' . $symbols[1]->name . ', ' . $symbols[2]->name . '. Obraćanje treba da bude u 2. licu jednine i da se izbjegava veliki broj infinitiva kako bi obraćanje bilo što ličnije.';
